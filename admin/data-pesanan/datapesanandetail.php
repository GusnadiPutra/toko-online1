...
$id	= $_GET['id'];
$qrydt="SELECT * FROM tb_pesanan INNER JOIN tb_pelanggan USING(id_pelanggan) WHERE id_pemesanan='$id' ORDER BY id_pemesanan='$id'";
$resultdt = $mysqli->query($qrydt);
$jumdt = mysqli_num_rows($resultdt);
    if($jumdt>0){
        while ($rowcust = $resultdt->fetch_assoc()) {

            $idpel = $rowcust['id_pelanggan'];
            ...
            $layananpsn = $rowcust['kurir_layanan'];
...
$qrybyr="SELECT * FROM tb_pembayaran WHERE id_pemesanan='$id' AND status_pembayaran='Confirm'";
$resultbyr = $mysqli->query($qrybyr);
$jumbyr = mysqli_num_rows($resultbyr);
if($jumbyr == 0) {
}else {
    $rowbyr=$resultbyr->fetch_assoc();
    $tgltrf = $rowbyr['tanggal_pembayaran'];
    $buktitrf = $rowbyr['bukti_transfer'];
    $status_b= $rowbyr['status_pembayaran'];
}
...
<p class="text-uppercase" ><label class="control-label col-md-8" ><?php echo $layananpsn; ?> </label></p>
        ...
        if($jumdata>0){
            while ($data = $result->fetch_assoc()) {
                ...
                <tr>
                    ...
                    <td>
                        <?php echo "Rp.".number_format($data['harga_pesanan'], 2, ',','.'); ?>
                    </td>
                    <td>
                        <?php echo $data['jumlah_produk']; ?>
                    </td>
                    <td>
                        <?php echo "Rp.".number_format($data['sub_total'], 2, ',','.'); ?>
                    </td>
                </tr>
                <?php
                $i++;
...
if($jumbyr == 0) {
    ...
    <label class="control-label col-md-8"><b><u><?php echo $status_b; ?> </u></b></label>
